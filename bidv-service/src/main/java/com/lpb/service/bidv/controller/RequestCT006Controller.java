...
@RestController
@Log4j2
public class RequestCT006Controller {
    @Autowired
    CT006Process ct006Process;

    @Operation(description = "Truy vấn bản tin CT006")
    @ApiResponses({
    ...
    @PostMapping("/api/v1/sequence-export-massage/")
    public ResponseEntity requestCT006(@Valid @NotNull @RequestBody LPBRequest<LPBDataRequestCT006> request) throws JsonProcessingException {
        ObjectMapper objectMapper = new ObjectMapper();
        String data = objectMapper.writeValueAsString(request);
        log.info(request.getMsgId() + " Request: " + data);
        try {
            return new ResponseEntity<>(ct006Process.requestCT006(request), HttpStatus.OK);
