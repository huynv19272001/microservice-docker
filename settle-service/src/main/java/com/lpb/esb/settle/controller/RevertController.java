...
@RestController
@RequestMapping(value = "/api/v1/revert", produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
@Log4j2
public class RevertController {
    @Autowired
    RevertService revertService;

    @PostMapping("/revert-bill")
    public ResponseEntity<?> RevertBillService(@RequestBody DataSettleDTO Req_DTO) {
        try {
            ResponseModel responseModel = revertService.excuteRevertService(Req_DTO);
