...
import com.lpb.esb.settle.service.SettleService;
...
public class SettleServiceImpl implements SettleService {
    ...
    private BaseRequestDTO mapObjectInit(DataSettleDTO REQ) {
        ...
        if (settleBillDTO.getPartner() != null) {
            com.lpb.esb.service.common.model.request.transaction.PartnerDTO partnerDTO = PartnerDTO.builder()
    ...
    @Override
    public ResponseModel excuteSettleService(DataSettleDTO REQ) {
...
//            Get thông tin kết nối
            ...
            if (appMsgId == null || appMsgId.length() < 1) {
                log.info(header.getMsgid() + " Error RQ SettleBill: 285 - Không thể thêm vào DB do lỗi: " +
            ...
            // Init SettleBill
            responseModel = TransactionService.initSettleBill(restTemplateLB, mapObjectInit(REQ));
            if (!responseModel.getResCode().getErrorCode().equals("00")) {
                log.info(appMsgId + "-" + header.getMsgid() + " Error RQ SettleBill: 285 - Không thể thêm vào DB do lỗi: " +
                    "TransactionService.initSettleBill(restTemplateLB, baseRequestDTO)");
                lpbResCode.setErrorCode("285");
                lpbResCode.setErrorDesc("Không thể thêm vào DB do lỗi: TransactionService.initSettleBill(restTemplateLB, baseRequestDTO)");
                lpbResCode.setRefCode(responseModel.getResCode().getErrorCode());
                lpbResCode.setRefDesc(responseModel.getResCode().getErrorDesc());
                responseModel = ResponseModel.builder().resCode(lpbResCode).data(REQ).build();
                return responseModel;
            }
            // xử lý với GD cần xác thực
            if (REQ.getBody().getSettleBill().getConfirmTrn() != null && REQ.getBody().getSettleBill().getConfirmTrn().equals("Y")) {
                lpbResCode.setErrorCode(ErrorMessage.SUCCESS.label);
                lpbResCode.setErrorDesc(ErrorMessage.SUCCESS.description);
                lpbResCode.setRefCode(responseModel.getResCode().getErrorCode());
                lpbResCode.setRefDesc(responseModel.getResCode().getErrorDesc());
                RES = REQ;
                //xử lý với GD không cần xác thực
            } else {
                //get SettleBill
                TransactionService.getSettleBill(restTemplateLB, mapObjectGetSettleBill(REQ, appMsgId));
