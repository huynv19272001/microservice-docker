...
@RestController
@RequestMapping(value = "chung-minh-nhan-dan", produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
public class ChungMinhNhanDanController {
    @Autowired
    CmndService cmndService;

    @RequestMapping(value = "search", method = RequestMethod.POST)
    public ResponseEntity<ResponseModel> search(@RequestBody EsbTctRequestDTO requestData) throws Exception {

        ResponseModel responseModel = cmndService.search(requestData);
