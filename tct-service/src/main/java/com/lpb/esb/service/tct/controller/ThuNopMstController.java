...
import com.lpb.esb.service.tct.service.ThuNopMstService;
...
@RestController
@RequestMapping(value = "thu-nop-mst", produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
public class ThuNopMstController {
    @Autowired
    ThuNopMstService thuNopMstService;

    @RequestMapping(value = "search", method = RequestMethod.POST)
    public ResponseEntity<ResponseModel> search(@RequestBody EsbTctRequestDTO requestData) throws Exception {

        ResponseModel responseModel = thuNopMstService.search(requestData);
