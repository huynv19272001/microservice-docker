...
public class VerifyPaymentController {
    @Autowired
    IVerifyPaymentProcess iVerifyPaymentProcess;

    @Autowired
    IInitVerifyPaymentResponse iInitVerifyPaymentResponse;

    @Operation(description = "API Verify Payment là quá trình xác minh thông tin tài khoản/thẻ của khách hàng")
    @ApiResponses({
    ...
    @PostMapping(value = "/api/v1/verifypayment")
    public ResponseEntity<?> verifyPayment(@RequestBody VerifyPaymentRequest verifyPaymentRequest) throws Exception {
        try {
            return new ResponseEntity<>(iVerifyPaymentProcess.initVerifyPaymentProcess(verifyPaymentRequest), HttpStatus.OK);
        } catch (ExceptionVerifyPayment e) {
            return new ResponseEntity<>(iInitVerifyPaymentResponse.initVerifyPaymentResponse
                (e.getVerifyPaymentRequest(), e.getInitVerifyPaymentRequest(),
                    e.getDataVerifyPaymentRequest(), e.getAppId()), HttpStatus.BAD_REQUEST);
        }
    }
}
