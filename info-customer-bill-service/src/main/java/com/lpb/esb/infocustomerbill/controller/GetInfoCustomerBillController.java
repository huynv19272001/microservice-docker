...
@Log4j2
public class GetInfoCustomerBillController {
    @Autowired
    GetInfoCustomerBillService getInfoCustomerBillService;

    @SuppressWarnings("rawtypes")
    @PostMapping("/get-info-customer-bill")
    public ResponseEntity<?> getInfoCustomerBill(@RequestBody DataQueryDTO Req_DTO) {
        try {
            ResponseModel responseModel = getInfoCustomerBillService.excuteGetInfoCustomerBill(Req_DTO);
